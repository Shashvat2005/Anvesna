  ...
  useEffect(() => {
    const fetchCommunities = async () => {
      ...
      try {
        ...
        if (snapshot.exists()) {
          const fetchedCommunities = Object.keys(snapshot.val()).map((key) => ({
            id: key,
            ...snapshot.val()[key],
          })) as Community[];
          setCommunities(fetchedCommunities);
        } else {
          console.log("No communities found in Realtime Database.");
        }
      } catch (error) {
          ...
          {isLoading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-sm text-muted-foreground">
                Check the database in Firebase Realtime Database if you expect to see communities.
              </p>
            </div>
          )}
        </div>
